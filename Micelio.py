...
class TokenBucket:
    ...
    def _recargar(self) -> None:
        ...
        self.tokens = min(self.capacidad, self.tokens + self.rho * (ahora - self.t_ultimo))
        self.t_ultimo = ahora
...
def volcar_telemetria(bloque: np.ndarray, etiqueta: str) -> None:
    ...
    try:
        ...
        try:
            ...
            pq.write_table(tabla, os.path.join(DIR_TELEMETRIA, f"telem_{etiqueta}.parquet"))
        except ImportError:
    ...
    try:
        while True:
            ...
            if IS_TESTNET:
                ...
                lam_ruido += theta * (mu_OU - lam_ruido) * dt_sim + sigma * math.sqrt(
                    dt_sim
                ) * np.random.randn()
